...
# To install, run:
# curl -L https://github.com/thierry-kosamera/NixOS/raw/master/install.sh | sudo bash -s -- -d <disk device> -h <host name>
...
main_repo="git@github.com:thierry-kosamera/NixOS.git"
config_repo="git@github.com:thierry-kosamera/emn-org-config.git"
...
(
  ...
  if [ "${can_authenticate}" -ne "0" ]; then
    ...
    echo -e "file in the NixOS-EMN-config repo:\n"
...
nix-shell --packages git --run "git config --global user.name 'EMN NixOS Robot'"
nix-shell --packages git --run "git config --global user.email 'nixos-emn@users.noreply.github.com'"
